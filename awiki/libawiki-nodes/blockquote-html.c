...
static t_awiki_html_fns blockquote_html_fns =
{
  (t_uchar *)"builtin 'blockquote' HTML type",
  awiki_blockquote_to_html
};

static t_awiki_html_type blockquote_html_type = { &blockquote_html_fns };


int
awiki_init_node_blockquote_html (void)
{
  static int initialized = -1;

  if (initialized < 0)
    {
      if (0 > awiki_register_html_type (Identifier (blockquote), &blockquote_html_type))
        return -1;
      initialized = 0;
    }
...
int
awiki_blockquote_to_html (t_awiki_html_state * state,
                     ...
                     t_buffer_point * output_pt)
{
  ...
  AWIKI_HTML_MULTILINE (state, output_pt, "blockquote");
...
/* arch-tag: Tom Lord Wed Nov 24 09:48:49 2004 (libawiki-nodes/blockquote-html.c)
 */
